...
void RH_ASK::timerSetup()
{
 ...
 #elif defined(_arm_) && defined(CORE_TEENSY)
    ...
    OpenTimer1(T1_ON | T1_PS_1_1 | T1_SOURCE_INT, (F_CPU / 😎 / _speed);
...
bool RH_ASK::send(const uint8_t* data, uint8_t len)
{
    ...
    // Caution: VW expects the ones_complement of the CCITT CRC-16 as the FCS
...
#if (RH_PLATFORM == RH_PLATFORM_ARDUINO) && defined(_arm_) && defined(CORE_TEENSY)	
void TIMER1_COMPA_vect(void)
{
    thisASKDriver->handleTimerInterrupt();
}

#elif (RH_PLATFORM == RH_PLATFORM_ARDUINO) && defined(_arm_)
// Arduino Due
void TC1_Handler()
{
    TC_GetStatus(TC0, 1);
